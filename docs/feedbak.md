...
**`CH11-KP1-01`**: 某仪表放大电路，要求输入电阻大，输出电流稳定，应选 ________ 型电路。
...
**`CH11-KP1-02`**: 对于放大电路，所谓开环是指 ________ 。
...
**`CH11-KP1-03`**: 放大器深度负反馈的条件是 ________ 。
...
**`CH11-KP1-04`**: 一个负反馈放大电路采样电压返回电流，则以下叙述正确的是 ________ 。
...
**`CH11-KP1-05`**: 判断下图所示电路 (a) 和 (b) 引入的反馈分别是 ________ 和 ________ ，其反馈系数分别为 ________ 和 ________ 。

<img src="./feedbak.assets/12-1-5.png" alt="12-1-5" style="zoom: 40%;" />

> 电压并联负反馈      电压串联负反馈      $-\frac{1}{R_f}$      $\frac{R_s}{R_s+R_f}$

**`CH11-KP1-06`**: 某传感器产生的是电压信号（几乎不能提供电流），经过放大后希望输出电压与信号成正比，这时放大电路应选 ________ 。
...
**`CH11-KP1-07`**: 判断下图所示电路中包含电阻 $R_f$ 的反馈网络的反馈类型为 ________ 。
...
**`CH11-KP1-08`**: 射极跟随器是典型的 ________ 负反馈。
...
**`CH11-KP1-09`**: 判断下图所示电路的反馈类型为 ________ 。
...
**`CH11-KP1-10`**: 如下图所示电路，要求达到以下目的，需要如何构建反馈网络。
...
**`CH11-KP1-11`**: 对于电压串联负反馈电路，由于反馈的引入，电路的 ________ 。
...
**`CH11-KP1-12`**: 以下对于负反馈结构特性的描述，错误的是 ________ 。
...
**`CH11-KP1-13`**: 设某个放大器开环时 $\frac{d|A_v|}{|A_V|}$ 为 20%，若要求 $\frac{d|A_{vf}|}{|A_{Vf}|}$ 不超过 1%，且 $|A_{vf}|=100$，则要求开环增益 $|A_v|$ 大于 ________ ，反馈系数 $|F|$ 大于 ________ 。

> 2000      0.0095（0.01）。采用深度负反馈近似，负反馈缩放因子 $(1+A\beta) \approx A\beta$， $\beta = 1/100$，所以开环增益 A =2000。  

**`CH11-KP1-14`**: 在输入量不变的情况下，若引入反馈后 ________ ，则说明引入的反馈是负反馈。
...
**`CH11-KP1-15`**: 由于射极跟随器引入了电压串联负反馈，因此电压放大倍数很小。但这种电路具有输入电阻 ________ （大/小）、输出电阻 ________ （大/小）的突出特点。

> 大  小。射极跟随电路的负反馈解释。

**`CH11-KP1-16`**: 开环增益为*A*、低频截止频率为 $f_L$ 的单极点放大器，若引入反馈系数为 *F* 的纯电阻反馈网路，则其闭环低频截止频率变为________。
...
**`CH11-KP2-03`**: 分析如下图所示的负反馈电路，其中输入信号 $V_s$ 的直流偏置电压为 0 V，$Q_3$ 的 $V_{BE}$ ≈ 0.7 V。
...
**`CH11-KP2-04`**: 已知一个负反馈放大电路的基本放大电路的幅频和相频特性如下图所示，反馈网络由纯电阻组成。若要求电路稳定工作，即不产生自激振荡，则反馈系数（$20\lg \left| {\dot F} \right|$）的上限值为<u>__  __</u>。
