...
> 关联参考方向指电流的参考方向从电压参考方向的正端流入，与实际电流电压方向无关。
...
> 关联参考方向时，计算功率为正，表示消耗功率；计算功率为负，表示提供功率。



**`CH01-KP1-03`**: 下列电路中，p1、p4 为未知类型元件，请计算每个元件消耗或提供的功率，并验证功率守恒。
...
> <img src="./concepts.assets/image-20221014135455079.png" alt="image-20221014135455079" style="zoom:40%;" />
...
**`CH01-KP2-02`**: 对于一个恒定容值的电容器，其总电荷量加倍时，其储存的能量 ________ 。
...
> $Q=CV$，对某一电容，$Q$ 加倍即 $V$ 加倍，而能量 = $\dfrac{1}{2}CV^2$，所以能量为原来的 4 倍。
...
> A 和 B 显然是正确的，电容储能 = $\dfrac{1}{2}CV^2$，电感储能 = $\dfrac{1}{2}LI^2$，故也不能突变，也是正确的。
...
> 菱形为受控源；标示箭头故为电流源；其值为 5v，即 5 倍的 2 $\Omega$ 电阻上的电压，故为电压控制电流源。
...
> 独立电源指电源的值是独立的，不受电路中其他量的控制，但这一独立的值可以是常数，也可以是时间的函数（如正弦电源）；理想电源，则其值不受负载影响。
...
> 理想电压源串联一个阻值为 0 的内阻；理想电流源并联一个阻值为正无穷大的内阻；若非理想，则上述内阻非零。
...
> 标示箭头，则为电流源；值为 $6i_0$，表示受电流控制；故为电流控制电流源。
